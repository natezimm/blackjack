...
describe('BlackjackGame', () => {
  ...
  it('shows the resume prompt when a saved hand exists', async () => {
    localStorage.setItem(
      ...
      JSON.stringify({
        playerHands: [
          { cards: [{ value: 'K', suit: 'Spades' }], isTurn: true, bet: 25 },
        ],
        dealerHand: [],
        currentBet: 25,
        bettingOpen: false,
    ...
    getState.mockResolvedValue({
      data: {
        playerHands: [
          { cards: [{ value: 'Q', suit: 'Hearts' }], isTurn: true, bet: 25 },
        ],
      },
    });
  ...
  it('places a bet through the API and updates the current bet display', async () => {
    ...
    await waitFor(() =>
      expect(within(betRow).getByText('$5')).toBeInTheDocument()
    );
  });
  ...
  it('blocks bets that exceed the available balance and shows a message', async () => {
    ...
    await waitFor(() =>
      expect(within(balanceRow).getByText('$5')).toBeInTheDocument()
    );
    ...
    expect(
      screen.getByText("Easy, high roller. That bet's bigger than your stack.")
    ).toBeInTheDocument();
  });
  ...
  it('handles resume action', async () => {
    ...
    await waitFor(() =>
      expect(screen.queryByText(/Resume your game/i)).not.toBeInTheDocument()
    );
  });
  ...
  it('opens and closes stats modal', async () => {
    ...
    const statsCloseButton = closeButtons.find((btn) =>
      btn.closest('.settings-modal-content')
    );
    if (statsCloseButton) {
      await act(async () => {
  ...
  it('shows game over message when balance is 0', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText(/Bankroll empty/i)).toBeInTheDocument()
    );
  });
  ...
  it('createUpdateBalanceAndStats handles fallback highest bankroll', () => {
    ...
    expect(persistStats).toHaveBeenCalledWith(
      expect.objectContaining({ highestBankroll: 250 })
    );
  });
  ...
  it('createUpdateStatsWithOutcome handles missing payout and balance', () => {
    const setStats = jest.fn();
    const persistStats = jest.fn();
    setStats.mockImplementation((fn) =>
      fn({
        highestBankroll: 100,
        currentWinStreak: 2,
        sessionHandsWon: 3,
  ...
  it('createUpdateStatsWithOutcome resets streak on loss', () => {
    ...
    expect(persistStats).toHaveBeenCalledWith(
      expect.objectContaining({ currentWinStreak: 0 })
    );
  });
    ...
    hydrateState(
      {
        balance: 800,
        playerHands: [
          { cards: [{ value: '5', suit: 'Hearts' }], isTurn: true, bet: 10 },
        ],
        dealerHand: [{ value: 'K', suit: 'Hearts' }],
        gameOver: false,
        bettingOpen: true,
  ...
  it('handles startGame with balance from API', async () => {
    startGame.mockResolvedValue({
      data: {
        playerHands: [
          { cards: [{ value: 'K', suit: 'Hearts' }], isTurn: true, bet: 10 },
        ],
